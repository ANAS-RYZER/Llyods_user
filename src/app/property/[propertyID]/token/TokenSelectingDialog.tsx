...
export default function TokenSelectingDialog({
  open,
  onOpenChange,
  onSubmit,
  property,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        className="w-full max-w-[450px] h-[520px] overflow-y-auto p-4 rounded-xl"
      >
        <DialogHeader className="border-b border-gray-200">
          ...
          <DialogDescription className="flex items-center gap-1 text-sm ">
            ...
            <span className="text-green-500 flex items-center gap-2">
              <TrendingUpIcon size={16} /> 1 Property Token ={" "}
        ...
        <div className="space-y-4">
          <div className="bg-gray-100 p-4 border border-gray-200 space-y-2 rounded-xl">
            <div className="flex justify-between ">
              ...
              <div className="flex items-center">
                ...
                <span className="text-sm text-yellow-700">
                  Tokens Available :{" "}
                  {property?.tokenInformation?.availableTokensToBuy - quantity}
                </span>
              </div>
            </div>
            <div className="flex items-center justify-between">
              ...
              <div className="flex flex-col items-center justify-center">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    type="button"
                    className="bg-white hover:bg-primary/20 rounded-full cursor-pointer shadow p-3 flex items-center justify-center"
                    onClick={() => {
                      if (quantity > minimuimToken) {
                        ...
                        setTokenError(
                          `You must select at least ${minimuimToken} tokens.`
                        );
                      }
                    }}
                  >
                    ...
                    onChange={(e) => {
                      ...
                      if (!isNaN(val)) {
                        ...
                        if (val < minimuimToken) {
                          setTokenError(
                            `Minimum ${minimuimToken} tokens required.`
                          );
                        } else if (val > maximumToken) {
                          setTokenError(
                            `You can only buy up to ${maximumToken} tokens.`
                          );
                        } else {
                          setTokenError(null);
                        }
                    ...
                    onBlur={(e) => {
                      ...
                      if (!isNaN(val)) {
                        if (val < minimuimToken) {
                          ...
                          setTokenError(
                            `Minimum ${minimuimToken} tokens to required to invest.`
                          );
                        } else if (val > maximumToken) {
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (quantity < maximumToken) {
                        ...
                        setTokenError(
                          `Maximum Tokens to buy  ${maximumToken} you cant invest more than this .`
                        );
                      }
                    }}
                  >
            ...
            {tokenError && (
              <span className="text-red-500 text-sm">{tokenError}</span>
            )}
          ...
          <div className="grid grid-cols-2 gap-4 ">
            <div className="bg-gray-100 p-2 border border-gray-200 rounded-xl space-y-2">
              <div className="flex justify-between">
                <h1 className="text-md text-black">Annual Returns </h1>
                <Info size={16} />
              </div>
              <div className="flex flex-col gap-2">
                <h1 className="font-bold text-xl text-green-500">
                  {roundDownToOneDecimal(totalPrice * (18.5 / 100))}
                </h1>
                <h1 className="text-sm text-gray-500">
                  Based on 18.5% Est. ROI
                </h1>
              </div>
            </div>
            <div className="bg-gray-100 p-2 border border-gray-200 rounded-xl space-y-2">
              <div className="flex justify-between">
                <h1 className="text-md text-black">Dividend Yield</h1>
                <Calendar size={16} />
              </div>
              <div className="flex flex-col gap-2">
                <h1 className="font-bold text-xl text-green-500">
                  {activeTab === "monthly"
                    ? roundDownToOneDecimal(totalPrice * (8.5 / 100))
                    : roundDownToOneDecimal(totalPrice * ((8.5 * 12) / 100))}
                </h1>
                <div className="relative bg-gray-200 rounded-lg flex items-center justify-between px-1 py-1">
                  ...
                  <button
                    className={`relative z-10 w-1/2 text-sm font-medium ${
                      activeTab === "yearly" ? "text-white" : "text-gray-600"
                    }`}
